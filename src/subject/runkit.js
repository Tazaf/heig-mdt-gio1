...
export class RunkitController {
  ...
  static startRunkit() {
    ...
    let enabled =
      $globalRunkitConfig.length &&
      $globalRunkitConfig.attr("enabled") &&
      $globalRunkitConfig.attr("enabled").match(/^(1|y|yes|t|true)$/i);

    const $runkitConfig = $(
      ".remark-visible .remark-slide-content runkit:not([global])"
    );
    ...
    if ($runkitConfig.length) {
      ...
      disabled =
        $runkitConfig.attr("disabled") !== undefined &&
        $runkitConfig.attr("disabled") !== false;
    ...
    $(
      ".remark-visible .remark-slide-content code.javascript:not(.runkit)"
    ).each(function(i) {
